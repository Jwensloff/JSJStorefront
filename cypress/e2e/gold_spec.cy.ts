describe("gold", () => {
  beforeEach(() => {
    ...
    cy.visit("http://localhost:3000/gold");
  });
  it("should show gold page content", () => {
    //Header
    ...
    // Hero image
    cy.get('[data-test ="gold-page-hero"]').should("exist");
    cy.get('[data-test ="gold-page-hero"]').should(
      "have.attr",
      "alt",
      "A close up of neck and shoulders with gold necklace around neck",
    );
    cy.get('[data-test="gold-page-hero-text"]').should("contain", "Indulge");
  ...
  it("should show individual product upon click", () => {
    ...
    cy.get('[data-test="content-section"]').within(() => {
      ...
      cy.wait("@gold_product");
      cy.url().should("eq", "http://localhost:3000/product/7");
    });
    cy.go("back");
    cy.url().should("eq", "http://localhost:3000/gold");
  });
});
